...
def crossover(left, right):
    S1 = left [0:4] + right [4:8]
    S2 = right [0:4] + left [4:8]
    return (S1,S2)
...
def solution_validation(solution,num_nodes):   
    ...
    for s in range(num_nodes):
            if index < num_nodes  and s != 0:
                path_index =  solution[index * 2] * 2 + solution[index * 2 + 1] * 1
...
if __name__=='__main__':
    print("heuristic genetic algo for four UAV")
    start_time = time.time()
    num_nodes = 5 # incuding ground station
        ...
        for d in range(num_nodes):
            ...
            if s != d and d == 0: 
                for i in range(num_nodes):
                    for j in range(num_nodes):
                        if linksMat[i][j] == 1:
                            # Considering only 4 paths from UAVs to Ground Station. It may very depends number of hops in the network 
                            paths = list(nx.shortest_simple_paths(g, s, d))[:4]
