...
def crossover(left, right):
    S1 = left [0:5] + right [5:10]
    S2 = right [0:5] + left [5:10]
    return (S1,S2)
...
def min_total_RB(solution,num_nodes, RBMat):
   ...
   for s in range(num_nodes):
        if index < num_nodes  and s != 0: # no need to cosnider ground station because we are not sending data from ground station
            path_index = solution[index + 2] + solution[index + 1]
            selected_path = path_dict[f"{s}-{0}"][path_index]
...
if __name__=='__main__':
    print("heuristic genetic algo for five UAV")
    start_time = time.time()
    num_nodes = 6 # incuding ground station
                ...
                for i in range(num_nodes):
                    for j in range(num_nodes):
                        if linksMat[i][j] == 1:
                            ...
                            paths = list(nx.shortest_simple_paths(g, s, d))[:5]
